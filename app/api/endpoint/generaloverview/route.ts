...
import { formatMonthlyStatistics, formatYearlyStatistics } from '@/lib/formatting';
...
export async function GET(req: Request) {
  try {
    ...
    if (filter.includes("month")) { 
      const data = formatMonthlyStatistics(result.rows as {createdAt : string}[],filter === "last-month")
      const formattedData = []
      for ( const key in data) { 
        formattedData.push({date : key , calls : data[key]})
      }
      return NextResponse.json(formattedData);
    } else if (filter.includes("year")) { 
      const data = formatYearlyStatistics(result.rows as {createdAt : string}[])
      const formattedData = []
      for ( const key in data) { 
        //@ts-ignore
        formattedData.push({date : key , calls : data[key]})
