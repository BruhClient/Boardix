...
export default function PieChartComponent({endpointId , fieldType, title } : {endpointId : string,fieldType : string , title? : string}) {
  ...
  return (
    <Card>
      ...
      <CardContent className='h-full flex justify-center items-center min-h-[300px] '>
    {
        (!isLoading) ? (
            (data && data.length > 0) ? <PieGraph data={data} fieldType={fieldType} sum={"count"}/> : <div>Unsufficient data</div>
        )
