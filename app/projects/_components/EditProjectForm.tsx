...
const EditProjectForm = ({id,name,starred,icon} : {id : string,name : string ,starred : boolean,icon : string}) => {
  const form = useForm(
    { 
  ...
  const onSubmit = (values : CreateProjectPayload) => { 
    startTransition(() => { 
      updateProjectById(id,values).then((data) => { 
        if (!data) { 
    ...
    <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)} className='space-y-3'>
          <FormField
            ...
            />
            <div className='flex gap-2'>
              <Button type='submit' disabled={isPending}>{isPending ? <span className='flex items-center gap-3'><ClipLoader size={15}/> Editting Project...</span>: "Edit Project"}</Button>
              <FormField
                        control={form.control}
                        name ="starred"
                        render={({field}) => (
